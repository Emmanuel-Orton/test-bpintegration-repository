...
class CreateSowReviewTaskStepTest extends AbstractWorkflowStepTest {

	private static final String WF_PREVIOUS_TASK = "WF_PREVIOUS_TASK";
	private static final String TENANT_WS = "TENANT_WS";
	private static final String WF_CURRENT_TASK = "WF_CURRENT_TASK";
	...
	void executeStep_currentTask() throws Exception {
		...
		when(currentTask.getTaskDefinition()).thenReturn(TaskType.TELUS_WS_SOW_REVIEW.name());
		...
		verifySetTaskInfo(CURRENT_TASK_ID, TaskType.TELUS_WS_SOW_REVIEW.name(), TENANT_WS, true);
	}
	...
	void executeStep_hasNoCurrentTask() throws Exception {
		...
		verify(basedOnTheOtherTaskHandler, times(1)).createNewTaskFromOldOne(
				PREVIOUS_TENANT,
				PREVIOUS_TASK_ID,
				TaskType.TELUS_WS_SOW_REVIEW
		);
