...
class CreateSowApproveTaskStepTest extends AbstractWorkflowStepTest {
	...
	void executeStep_currentTask() throws Exception {
		execution = new ExecutionMockBuilder()
				...
				.mockGetMandatoryVariable(WF_CURRENT_TASK, TENANT_WS)
				.build();
		...
		when(workflowUtil.getTaskInfo(TENANT_WS)).thenReturn(currentTask);
		...
		verify(taskAttachmentsHandler).copyFirstAvailableAttachment(
				PREVIOUS_TENANT,
				TENANT_WS,
				PREVIOUS_TASK_ID,
				CURRENT_TASK_ID,
	...
	void executeStep_hasNoCurrentTask() throws Exception {
		execution = new ExecutionMockBuilder()
				...
				.mockGetMandatoryVariable(TASK_TELUS_WS_SOW_CREATE_ID, TASK_TELUS_WS_SOW_CREATE_ID)
				.build();
		...
		when(basedOnTheOtherTaskHandler
				.createNewTaskFromOldOne(TENANT_WS, TASK_TELUS_WS_SOW_CREATE_ID,
						TaskType.TELUS_WS_SOW_APPROVE)).thenReturn(newTaskFromOldOne);

		unit.executeStep(execution);

		verify(basedOnTheOtherTaskHandler, times(1))
				.createNewTaskFromOldOne(TENANT_WS, TASK_TELUS_WS_SOW_CREATE_ID,
